...
namespace App\Livewire\Admin\Inventory\SiteEngineer\StockRequest;
...
class StockRequestList extends Component
{
    ...
    public function render(): View
    {
        ...
        $storesQuery = Store::query()->active()->orderBy('name');
            $requestStores = (clone $storesQuery)->where('type', StoreType::PROJECT->value)->wherehas('project.engineers', function (Builder $query) {
                $query->where('users.id', auth()->id());
             })->get(['id', 'name', 'code', 'type']);
            $officeStores = (clone $storesQuery)->where('type', StoreType::OFFICE->value)->get(['id', 'name', 'code', 'type']);

        return view('livewire.admin.inventory.site-engineer.stock-request.stock-request-list', [
            ...
            'requesterStores' => $requestStores,
            'officeStores' => $officeStores,
