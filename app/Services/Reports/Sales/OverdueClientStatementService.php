...
class OverdueClientStatementService
{
    public function build(array $filters): array
    {
        ...
        $clientData = $sales->groupBy('customer_id')->map(function ($customerSales) {
            ...
            // Only clients that have an outstanding balance AND at least one overdue installment.
            ->filter(fn ($row) => $row['total_due'] > 0 && $row['overdue_count'] >= 1)
            ->values()
            ->toArray();
        ...
        $meta = [
            ...
            'report_title' => 'Overdue Client Statement',
            'report_slug' => 'overdue-client-statement',
            'generated_at' => now()->format('d-M-Y H:i A'),
            'generated_by' => auth()->user()?->name ?? 'System',
            'from_date' => $filters['from_date'] ?? '-',
            'to_date' => $filters['to_date'] ?? '-',
            'file_name' => 'overdue-client-statement-' . now()->format('Y-m-d-His'),
            'notes' => $filters['notes'] ?? '',
        ];

        return [
            'title' => 'Overdue Client Statement',
            'slug' => 'overdue-client-statement',
            'columns' => [
