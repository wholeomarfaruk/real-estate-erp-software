<div x-data x-init="$store.pageName = { name: 'Collections', slug: 'accounts-collections' }">
    ...
    <div class="mt-4 rounded-2xl border border-gray-200 bg-white">
        <div class="px-5 py-4 sm:px-6 sm:py-5">
            <div class="grid grid-cols-1 gap-3 lg:grid-cols-12">
                ...
                <div class="lg:col-span-2">
                    <select wire:model.live="collectionTypeFilter"
                        class="h-11 w-full rounded-lg border border-gray-300 px-3 text-sm text-gray-800 focus:border-indigo-500 focus:outline-none">
                            ...
                            @forelse ($collections as $collection)
                                <tr>
                                    <td class="px-5 py-4 text-sm font-medium text-gray-800">
                                        {{ $collection->collection_no ?: 'Auto' }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ optional($collection->date)->format('d M, Y') }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $collection->method?->label() ?? 'N/A' }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $collection->collection_type?->label() ?? 'N/A' }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        <p>Dr: {{ $collection->collectionAccount?->name ?? 'N/A' }}</p>
                                        <p class="text-xs text-gray-500">Cr:
                                            {{ $collection->targetAccount?->name ?? 'N/A' }}</p>
                                    </td>
                                    <td class="px-5 py-4 text-sm text-gray-700">{{ $collection->payer_name ?: 'N/A' }}
                                    </td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                    ...
                                    <td class="px-5 py-4 text-right text-sm font-medium text-gray-700">
                                        {{ number_format((float) $collection->amount, 2) }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $collection->creator?->name ?? 'N/A' }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        <span
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="relative flex justify-end" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" @click.away="open = false" style="display: none;"
                                                ...
                                                <a href="{{ route('admin.accounts.collections.print', $collection) }}"
                                                    target="_blank"
                                                    class="flex w-full items-center rounded-lg px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                    Print
                                                </a>

                                                <a href="{{ route('admin.accounts.collections.pdf', $collection) }}"
                                                    class="flex w-full items-center rounded-lg px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                    Download PDF
                                                </a>

                                                <button type="button"
                                                    wire:click="openAttachmentModal({{ $collection->id }})"
                                                    class="flex w-full items-center rounded-lg px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                    Attachments
                                                </button>
                                                ...
                                                @can('accounts.collection.edit')
                                                    <button type="button"
                                                        wire:click="openEditModal({{ $collection->id }})"
                                                        class="flex w-full items-center rounded-lg px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                        Edit
                                                    </button>
                                                @endcan
                            ...
                            @empty
                                <tr>
                                    <td colspan="11" class="px-5 py-12 text-center">
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">Try changing filters or create a
                                            collection.</p>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <x-modal wire:model="showFormModal" max-width="2xl">
        <div class="p-6">
            <h2 class="text-xl font-bold text-gray-900">{{ $editingId ? 'Edit Collection' : 'Create Collection' }}
            </h2>

            <form wire:submit.prevent="save" class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    <label class="text-sm font-medium text-gray-700">Collection No</label>
                    <input type="text" wire:model.defer="collection_no"
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Collection Type <span
                            class="text-rose-500">*</span></label>
                    <select wire:model.defer="collection_type"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                        @foreach ($collectionTypes as $type)
                            <option value="{{ $type->value }}">{{ $type->label() }}</option>
                        @endforeach
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Collection Account (Debit) <span
                            class="text-rose-500">*</span></label>
                    <select wire:model.defer="collection_account_id"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                        <option value="">Select account</option>
                        @foreach ($types as $accountType)
                            @if (($groupedAccounts[$accountType->value] ?? collect())->count())
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Target Account (Credit) <span
                            class="text-rose-500">*</span></label>
                    <select wire:model.defer="target_account_id"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                        <option value="">Select account</option>
                        @foreach ($types as $accountType)
                            @if (($groupedAccounts[$accountType->value] ?? collect())->count())
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Amount <span
                            class="text-rose-500">*</span></label>
                    <input type="number" step="0.01" min="0" wire:model.defer="amount"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none"
                        placeholder="0.00">
                    @error('amount')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Payer Name</label>
                    <input type="text" wire:model.defer="payer_name"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none"
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Reference Type</label>
                    <input type="text" wire:model.defer="reference_type"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none"
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700">Reference ID</label>
                    <input type="number" min="1" wire:model.defer="reference_id"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none"
                ...
                <div class="sm:col-span-2">
                    <x-media-picker-field field="attachment_ids" :value="$attachment_ids" placeholder="Add attachments"
                        :multiple="true" type="all" label="Attachments" required="false" />
    ...
    {{-- <div x-cloak x-data="{ open: @entangle('showAttachmentModal') }" x-show="open" x-transition
        ...
        <div class="w-full max-w-3xl rounded-lg bg-white p-6 shadow-lg">
            <div class="flex items-start justify-between">
                ...
                <button type="button" @click="open = false; $wire.closeAttachmentModal()"
    ...
    <x-modal wire:model="showAttachmentModal" max-width="2xl">
        <div class="p-6">
            <h2 class="text-xl font-bold text-gray-900">Collection Attachments</h2>
