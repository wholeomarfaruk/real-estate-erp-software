<div x-data x-init="$store.pageName = { name: 'Bank Accounts' }">
    ...
    <div class="grid grid-cols-2 gap-3 mb-4 sm:grid-cols-3 xl:grid-cols-5">
        ...
        @foreach($typeBalances as $typeKey => $info)
            <div class="rounded-xl border border-gray-200 bg-white px-4 py-3"
                ...
                <p class="mt-1 text-xs text-gray-400">{{ $info['count'] }} {{ $info['count'] === 1 ? 'account' : 'accounts' }}</p>
            </div>
        @endforeach
    </div>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div class="relative">
            ...
            <input type="text" wire:model.live.debounce.400ms="search"
                placeholder="Search bank, code, A/C…"
        ...
        @forelse($accounts as $account)
            ...
            <div class="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 cursor-pointer transition hover:shadow-md hover:-translate-y-px"
                ...
                <div class="flex items-start justify-between gap-3 pl-2">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5 truncate max-w-[160px]">{{ $account->branch ?: '—' }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col items-end gap-1 shrink-0">
                        <span class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9.5px] font-bold uppercase tracking-wide {{ $typeBg }}">
                            {{ $typeLabel }}
                        </span>
                        <span class="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide
                            {{ $account->status === 'active' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-gray-100 border-gray-200 text-gray-500' }}">
                            <span class="h-1.5 w-1.5 rounded-full {{ $account->status === 'active' ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>
                ...
                <div class="mt-3 pl-2 flex items-center justify-between">
                    ...
                    @if($account->code)
                        <span class="font-mono text-[9.5px] font-semibold bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $account->code }}</span>
                    @endif
                </div>
                ...
                <div class="mt-1.5 pl-2 flex items-baseline gap-2">
                    ...
                    <span class="font-mono text-xs text-gray-600 font-medium tracking-wide">{{ $account->masked_ac_number }}</span>
                </div>

                {{-- Balance --}}
                <div class="mt-2 pl-2">
                    <span class="text-xs text-gray-400 mr-1">{{ $account->account?->code ? 'BDT' : '' }}</span>
                    <span class="text-2xl font-semibold text-gray-900 tracking-tight">{{ number_format($account->computed_balance, 2) }}</span>
                </div>

                {{-- Flows --}}
                <div class="mt-3 pl-2 grid grid-cols-2 gap-2 border-t border-dashed border-gray-200 pt-3">
                    <div>
                        <p class="flex items-center gap-1 text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">
                            <svg class="h-2.5 w-2.5 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                                <line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/>
                            </svg>
                            Today In
                        </p>
                        <p class="mt-0.5 font-mono text-xs font-semibold text-emerald-600">+{{ number_format($account->today_inflow, 0) }}</p>
                    </div>
                    <div>
                        <p class="flex items-center gap-1 text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">
                            <svg class="h-2.5 w-2.5 text-rose-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round">
                                <line x1="12" y1="5" x2="12" y2="19"/><polyline points="19 12 12 19 5 12"/>
                            </svg>
                            Today Out
                        </p>
                        <p class="mt-0.5 font-mono text-xs font-semibold text-rose-600">−{{ number_format($account->today_outflow, 0) }}</p>
                    </div>
                </div>
                ...
                <div class="mt-3 pl-2 flex items-center justify-between border-t border-gray-100 pt-2.5">
                    <p class="text-[10.5px] text-gray-400">
                        ...
                        @if($account->account_id)
                            <span class="font-mono text-gray-600">{{ optional(\App\Models\Transaction::where('account_id', $account->account_id)->latest('datetime')->value('datetime'))->format('d M Y') ?? '—' }}</span>
                        @else
    ...
    {{-- ═══════════════════════════════════════════════════════════════════════
         DETAIL MODAL
    ═══════════════════════════════════════════════════════════════════════════ --}}
    <div x-cloak x-data="{ open: @entangle('showDetailModal') }" x-show="open"
        ...
        class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
        ...
        <div x-show="open"
            ...
            <div class="flex items-center justify-between gap-3 border-b border-gray-100 px-6 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">{{ $va->branch ?: '—' }} · {{ $va->code ?: 'No code' }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    @can('accounts.chart.edit')
            ...
            <div class="mx-6 mt-5 rounded-xl px-6 py-5 text-white flex items-center justify-between gap-4"
                ...
                <div>
                    <p class="text-[10.5px] font-semibold uppercase tracking-widest text-white/60">Current Balance</p>
                    <p class="mt-2 text-4xl font-semibold tracking-tight">
                        <span class="text-sm text-white/60 mr-1 align-top mt-1 inline-block">BDT</span>{{ number_format($va->computed_balance, 2) }}
                    </p>
                ...
                <div class="flex gap-5 shrink-0">
                    <div class="text-right">
                        ...
                        <p class="mt-1 font-mono text-sm font-semibold text-emerald-300">+{{ number_format($va->today_inflow ?? 0, 0) }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-[9.5px] uppercase tracking-wide text-white/60">Today Out</p>
                        <p class="mt-1 font-mono text-sm font-semibold text-rose-300">−{{ number_format($va->today_outflow ?? 0, 0) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">
                ...
                <div>
                    <h3 class="mb-3 text-[10.5px] font-bold uppercase tracking-widest text-gray-400">Account Details</h3>
                ...
                <div class="space-y-4">
                    ...
                    @if($va->account)
                        <div>
                            <h3 class="mb-2 text-[10.5px] font-bold uppercase tracking-widest text-gray-400">Chart of Accounts</h3>
                            <div class="flex items-center gap-3 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3">
                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-900">
                                    <svg class="h-4 w-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M2.25 18.75a60.09 60.09 0 0 1 15.549-15.549"/><path d="M18.75 2.25v4.5m0-4.5h-4.5"/>
                                    </svg>
                                </div>
                                <div>
                                    <p class="font-mono text-sm font-semibold text-gray-900">{{ $va->account->code }} — {{ $va->account->name }}</p>
                                    <p class="text-xs text-gray-500 mt-0.5">{{ strtoupper($va->account->type?->value ?? '') }} · {{ strtoupper($va->account->sub_type ?? '') }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($va->note)
                        <div>
                            <h3 class="mb-1.5 text-[10.5px] font-bold uppercase tracking-widest text-gray-400">Notes</h3>
                    ...
                    @if(!empty($va->files))
                        <div>
                            <h3 class="mb-2 text-[10.5px] font-bold uppercase tracking-widest text-gray-400">
                                Documents
                                <span class="ml-1 rounded bg-gray-100 px-1.5 py-0.5 font-mono text-[9px] text-gray-600">{{ count($va->files) }}</span>
                            </h3>
            ...
            @if(!empty($va->recent_transactions) && $va->recent_transactions->count())
                <div class="border-t border-gray-100 px-6 pb-6">
                    <h3 class="mb-3 mt-4 text-[10.5px] font-bold uppercase tracking-widest text-gray-400">Recent Transactions</h3>
                    <div class="overflow-hidden rounded-lg border border-gray-200">
                        <table class="min-w-full text-xs">
                            <thead>
                                <tr class="border-b border-gray-100 bg-gray-50">
                                    <th class="px-3 py-2 text-left text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">Date</th>
                                    <th class="px-3 py-2 text-left text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">Notes</th>
                                    <th class="px-3 py-2 text-right text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">Debit</th>
                                    <th class="px-3 py-2 text-right text-[9.5px] font-semibold uppercase tracking-wide text-gray-400">Credit</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                @foreach($va->recent_transactions as $txn)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-3 py-2 font-mono text-gray-500">{{ \Carbon\Carbon::parse($txn->datetime)->format('d M Y') }}</td>
                                        <td class="px-3 py-2 text-gray-700 max-w-[180px] truncate">{{ $txn->notes ?: ($txn->name ?: '—') }}</td>
        ...
        <div class="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl overflow-y-auto max-h-[90vh]">
            ...
            <form wire:submit.prevent="save" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    <label class="text-sm font-medium text-gray-700">Account Type <span class="text-rose-500">*</span></label>
                    <div class="mt-1.5 flex flex-wrap gap-2">
                        @foreach($types as $typeOption)
                            <label class="cursor-pointer">
                                <input type="radio" wire:model.live="type" value="{{ $typeOption->value }}" class="sr-only">
                                <span class="inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-sm font-medium transition
                                    {{ $type === $typeOption->value ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-400' }}">
                                    {{ $typeOption->label() }}
                                </span>
                            </label>
                        @endforeach
                ...
                @if($type !== 'cash')
                    <div>
                        ...
                        <input type="text" wire:model.defer="ac_number" placeholder="Account number"
                            class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                        @error('ac_number') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                    </div>
                @endif

                {{-- Holder Name --}}
                <div>
                    <label class="text-sm font-medium text-gray-700">Holder Name <span class="text-rose-500">*</span></label>
                    <input type="text" wire:model.defer="holder_name" placeholder="Account holder"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                    @error('holder_name') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" wire:model.defer="branch" placeholder="Branch or location"
                        class="mt-1 h-10 w-full rounded-lg border border-gray-300 px-3 text-sm focus:border-indigo-500 focus:outline-none">
                    @error('branch') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>
