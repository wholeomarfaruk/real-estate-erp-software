<div x-data x-init="$store.pageName = { name: 'Product Brands' }">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr>
                                <td class="px-3 py-2 font-medium text-gray-800">
                                    <div class="flex items-center gap-3">
                                        <a class="block"
                                            href="{{ $brand->image_id ? file_path($brand->image_id) : 'https://ui-avatars.com/api/?name=' . urlencode($brand->name) . '&background=111827&color=fff&rounded=falese&bold=true' }}"
                                            data-fancybox
                                            data-caption="{{ $brand->name }} <br> {{ $brand->description }}">
                                            <img src="{{ $brand->image_id ? file_path($brand->image_id) : 'https://ui-avatars.com/api/?name=' . urlencode($brand->name) . '&background=111827&color=fff&rounded=falese&bold=true' }}"
                                ...
                                <td class="px-3 py-2 text-gray-500">{{ $brand->updated_at?->diffForHumans() }}</td>
                                <td class="px-3 py-2 flex gap-2">
                                    ...
                                    <button x-data="livewireConfirm()"
                                                    @click="confirmAction({id: {{ $brand->id }},method: 'delete',title: 'Are you sure?',text: 'This record & all related data will be permanently deleted!',confirmText: 'Yes, delete brand!',icon: 'warning'})"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-3 py-6 text-center text-gray-500">No brands yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
