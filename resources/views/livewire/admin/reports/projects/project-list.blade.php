<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
    <div class="max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-4xl font-bold text-slate-900">Project List</h1>
                <p class="text-slate-600 mt-2">All projects with type, location, area, budget, progress and counts.</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <p class="text-3xl font-bold text-slate-900 mt-2">{{ $report['summary']['total_projects'] }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <p class="text-sm font-medium text-slate-600">Total Budget</p>
                <p class="text-2xl font-bold text-slate-900 mt-2">{{ number_format((float)$report['summary']['total_budget'], 0) }}</p>
            </div>
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                    ...
                                    <td class="px-3 py-2.5 text-{{ $column['align'] }} text-slate-700 @if($column['key'] === 'name') font-medium @endif">
                                        @if($column['key'] === 'status')
                                            ...
                                            <span class="inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold
                                                ...
                                                @elseif($sv === 'ongoing') bg-indigo-100 text-indigo-700
                                                @elseif($sv === 'on hold') bg-amber-100 text-amber-700
                    ...
                    @if(count($report['rows']) > 0)
                        <tfoot>
                            <tr class="bg-slate-50 border-t-2 border-slate-300 font-semibold text-slate-900">
                                {{-- cols 1-7 --}}
                                <td class="px-3 py-2.5 text-left" colspan="7">Total ({{ $report['summary']['total_projects'] }} projects)</td>
                                {{-- col 8: Budget --}}
                                <td class="px-3 py-2.5 text-right">{{ number_format((float)$report['summary']['total_budget'], 0) }}</td>
                                {{-- col 9: Progress --}}
                                <td class="px-3 py-2.5"></td>
                                {{-- col 10: Properties --}}
                                <td class="px-3 py-2.5 text-center">{{ $report['summary']['total_properties'] }}</td>
                                {{-- col 11: Units --}}
