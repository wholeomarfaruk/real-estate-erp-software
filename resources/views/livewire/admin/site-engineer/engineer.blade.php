   ...
   <div x-data x-init="$store.pageName = { name: 'Manage Site Engineers' }">
                           ...
                           @foreach ($users as $userItem)
                               <tr class="*:text-gray-900 *:first:font-medium">
                                   ...
                                   <td class="px-3 py-2 whitespace-nowrap">
                                       {{ $userItem?->roles?->first()?->name ?? 'No Role' }}
                                   </td>
                                   <td class="px-3 py-2 whitespace-nowrap">
               ...
               <div class="mt-4">
                   <div class="flow-root">
                       <dl class="-my-3 divide-y divide-gray-200 text-sm">
                           ...
                           <div class="grid grid-cols-1 gap-1 py-3 sm:grid-cols-3 sm:gap-4">
                               <dt class="font-medium text-gray-900">Assigned Projects</dt>

                               <dd class="text-gray-700 sm:col-span-2">
                                   <label for="projectIds">
                                       <select wire:model.live="projectIds" multiple id="projectIds" name="projectIds"
                                           class="mt-0.5 w-full px-3 py-2 rounded border border-gray-300 shadow-sm sm:text-sm">
                                           @foreach ($projects as $item)
                                               <option value="{{ $item->id }}"
                                                   {{ $user?->projects->contains($item->id) ? 'selected' : '' }}>
                                                   {{ $item->name }}
                                               </option>
                                           @endforeach
                                       </select>
