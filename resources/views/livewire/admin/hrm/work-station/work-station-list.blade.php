<div x-data x-init="$store.pageName = { name: 'Work Stations', slug: 'hrm-work-stations' }">
    <div class="flex flex-wrap justify-between gap-4">
        <div>
            <h1 class="text-lg font-bold text-gray-700">Work Stations</h1>
            <p class="text-sm text-gray-500">Manage work station locations for employee assignment.</p>
        </div>
        ...
        <nav>
            <ol class="flex items-center gap-1.5 text-sm text-gray-500">
                ...
                <li>/</li>
                <li class="text-gray-700">Work Stations</li>
            </ol>
        </nav>
    </div>
                    ...
                    <table class="min-w-full">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($workStations as $workStation)
                                <tr>
                                    <td class="px-5 py-4 text-sm font-medium text-gray-800">{{ $workStation->name }}</td>
                                    <td class="px-5 py-4 text-sm text-gray-700">{{ $workStation->code ?: 'N/A' }}</td>
                                    <td class="px-5 py-4">
                                        <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {{ $workStation->status ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                            {{ $workStation->status ? 'Active' : 'Inactive' }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4 text-right text-sm text-gray-700">{{ $workStation->employees_count }}</td>
                                    <td class="px-5 py-4">
                                        <div class="relative flex justify-end" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" @click.away="open = false" style="display: none;" class="absolute right-0 z-40 mt-10 w-44 origin-top-right rounded-md border border-zinc-200 bg-white p-1 shadow-lg">
                                                @can('hrm.work-stations.update')
                                                    <button type="button" wire:click="openEditModal({{ $workStation->id }})" @click="open = false" class="flex w-full items-center rounded-lg px-3 py-2 text-left text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                        Edit
                                                    </button>
                                                @endcan
                                                @can('hrm.work-stations.delete')
                                                    <button
                                                        type="button"
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-5 py-12 text-center">
                                        <p class="text-sm font-medium text-gray-700">No work stations found.</p>
                                        <p class="mt-1 text-xs text-gray-500">Try changing search or create a work station.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div x-cloak x-data="{ open: @entangle('showFormModal') }" x-show="open" x-transition class="fixed inset-0 z-50 grid place-content-center bg-black/50 p-4">
        <div class="w-full max-w-xl rounded-lg bg-white p-6 shadow-lg">
            <div class="flex items-start justify-between">
                <h2 class="text-xl font-bold text-gray-900">{{ $editingId ? 'Edit Work Station' : 'Create Work Station' }}</h2>
