    ...
    <div class="mt-4 rounded-2xl border border-gray-200 bg-white">
        <div class="px-5 py-4 sm:px-6 sm:py-5">
            ...
            @can('inventory.supplier.create')
                <div class="mt-4">
                    <a href="{{ route('admin.inventory.suppliers.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-gray-800">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($suppliers as $supplier)
                                <tr>
                                    ...
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $supplier->address ? \Illuminate\Support\Str::limit($supplier->address, 40) : 'N/A' }}
                                    </td>
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="relative flex justify-end" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" @click.away="open = false" style="display: none;" x-transition class="absolute right-0 z-40 mt-10 w-48 origin-top-right rounded-md border border-zinc-200 bg-white p-1 shadow-lg">
                                                @can('inventory.supplier.update')
                                                    <a href="{{ route('admin.inventory.suppliers.edit', $supplier) }}" class="flex items-center rounded-lg px-3 py-2 text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                        Edit
                                                    </a>
                                                @endcan
                                                ...
                                                @can('inventory.supplier.update')
                                                    ...
                                                    >
                                                        {{ $supplier->status ? 'Mark Inactive' : 'Mark Active' }}
                                                    </button>
                                                @endcan

                                                @can('inventory.supplier.delete')
                                                    <button
                                                        type="button"
                                                        x-data="livewireConfirm"
                                                        @click="confirmAction({
                                ...
                                <tr>
                                    <td colspan="6" class="px-5 py-12 text-center">
                                        <div class="mx-auto max-w-sm">
                                            ...
                                            <p class="mt-1 text-xs text-gray-500">Try changing the search/filter or create a new supplier.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
