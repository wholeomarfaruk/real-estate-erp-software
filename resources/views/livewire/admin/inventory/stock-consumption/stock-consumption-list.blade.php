<div x-data x-init="$store.pageName = { name: 'Stock Consumption', slug: 'stock-consumptions' }">
    <div class="flex flex-wrap justify-between gap-6">
        ...
        <nav>
            <ol class="flex items-center gap-1.5">
                ...
                <li class="text-sm text-gray-800">Stock Consumption</li>
            </ol>
        </nav>
    </div>
        ...
        <div class="px-5 py-4 sm:px-6 sm:py-5">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-6">
                <div class="md:col-span-2">
                    ...
                    <input id="search" type="text" wire:model.live.debounce.400ms="search" placeholder="Search by no"
            ...
            @can('inventory.stock.consumption.create')
                <div class="mt-4">
                    <a href="{{ route('admin.inventory.stock-consumptions.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-gray-800">
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-b border-gray-100">
                                <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Document</th>
                                <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Store / Project</th>
                                <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Status</th>
                                <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Created By</th>
                                <th class="px-5 py-3 text-left text-xs font-medium text-gray-500">Posted By</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($consumptions as $consumption)
                                @php
                                    $statusClass = match ($consumption->status?->value) {
                                ...
                                <tr>
                                    <td class="px-5 py-4">
                                        <p class="text-sm font-medium text-gray-800">{{ $consumption->consumption_no }}</p>
                                        <p class="text-xs text-gray-500">{{ optional($consumption->consumption_date)->format('d M, Y') }}</p>
                                    </td>

                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        <p>{{ $consumption->store?->name ?? 'N/A' }}</p>
                                    ...
                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $consumption->creator?->name ?? 'N/A' }}
                                    </td>

                                    <td class="px-5 py-4 text-sm text-gray-700">
                                        {{ $consumption->poster?->name ?? 'N/A' }}
                                    </td>
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="relative flex justify-end" x-data="{ open: false }">
                                            ...
                                            <div x-show="open" @click.away="open = false" style="display: none;" x-transition class="absolute right-0 z-40 mt-10 w-52 origin-top-right rounded-md border border-zinc-200 bg-white p-1 shadow-lg">
                                                <a href="{{ route('admin.inventory.stock-consumptions.show', $consumption) }}" class="flex items-center rounded-lg px-3 py-2 text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                    View
                                                </a>

                                                @can('inventory.stock.consumption.update')
                                                    @if ($consumption->status?->value === 'draft')
                                                        <a href="{{ route('admin.inventory.stock-consumptions.edit', $consumption) }}" class="flex items-center rounded-lg px-3 py-2 text-sm text-zinc-700 transition hover:bg-zinc-100">
                                                            Edit
                                                        </a>
                                                    @endif
                                                @endcan
                                                    ...
                                                    @if ($consumption->status?->value === 'draft')
                                                        <button type="button" x-data="livewireConfirm"
                                                            @click="confirmAction({
                                                                ...
                                                                confirmText: 'Yes, post consumption'
                                                            })"
                                                ...
                                                @can('inventory.stock.consumption.delete')
                                                    @if ($consumption->status?->value === 'draft')
                                                        <button type="button" x-data="livewireConfirm"
                                                            @click="confirmAction({
                                                                id: {{ $consumption->id }},
                                                                method: 'deleteConsumption',
                                                                title: 'Delete this draft?',
                                                                text: 'Draft consumption and items will be removed.',
                                ...
                                <tr>
                                    <td colspan="6" class="px-5 py-12 text-center">
                                        <div class="mx-auto max-w-sm">
                                            <p class="text-sm font-medium text-gray-700">No stock consumption found</p>
                                            <p class="mt-1 text-xs text-gray-500">Try changing filters or create a new document.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
