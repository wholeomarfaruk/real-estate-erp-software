<div
    ...
    x-init="$store.pageName = { name: 'Notification Templates', slug: 'notification' }"
    ...
    <div style="padding:28px 24px 0;" class="flex items-end justify-between gap-4 flex-wrap">
        <div>
            <div style="font-size:11px;color:var(--ink-3);font-family:var(--mono);display:flex;gap:6px;align-items:center;margin-bottom:6px;">
                <span>Notifications</span><span style="opacity:.4">/</span><span style="color:var(--ink-1)">Templates</span>
            </div>
            <h1 style="font-size:24px;font-weight:600;letter-spacing:-.01em;margin:0;">Notification Templates</h1>
            <p style="margin-top:4px;font-size:13px;color:var(--ink-2);">Reusable title/body pairs with smart variables for in-app + push notifications.</p>
        </div>
    ...
    <div style="padding:20px 24px 80px;">
        ...
        {{-- KPI --}}
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:1px;background:var(--rule);border:1px solid var(--rule);border-radius:10px;overflow:hidden;margin-bottom:20px;max-width:400px;">
            @foreach([['label'=>'Total','value'=>$kpi['total'],'fg'=>'var(--ink-1)'],['label'=>'Active','value'=>$kpi['active'],'fg'=>'#065F46']] as $s)
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid var(--rule);" class="hover:bg-gray-50">
                        <td style="padding:12px 16px;">
                            ...
                            <div style="font:11px var(--mono);color:var(--ink-3);margin-top:2px;">{{ Str::limit($tpl->title,50) }}</div>
                        </td>
                        <td style="padding:12px 16px;">
                            <span style="padding:3px 9px;border-radius:20px;font:600 10px 'Inter',sans-serif;background:#DBEAFE;color:#1D4ED8;">{{ $tpl->type->label() }}</span>
                        </td>
                        <td style="padding:12px 16px;">
                ...
                <form wire:submit="save">
                    <div class="flex flex-col gap-4">
                        <div>
                            ...
                            <input wire:model="fName" type="text" placeholder="e.g. Welcome Message"
                        ...
                        <div>
                            ...
                            @error('fType') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input wire:model="fTitle" type="text" placeholder="Hi {name}, welcome!"
                                style="width:100%;padding:8px 12px;border:1px solid var(--rule);border-radius:7px;font:13px 'Inter',sans-serif;background:var(--canvas);outline:none;box-sizing:border-box;">
                        ...
                        <div>
                            ...
                            <div style="font:11px var(--mono);color:var(--ink-3);margin-bottom:6px;">Use {"{"}name{"}"} as a variable — substituted for single-user sends</div>
                            <textarea wire:model="fBody" rows="6" placeholder="Hi {name}, your account is ready..."
