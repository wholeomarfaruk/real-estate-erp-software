        ...
        <nav class="-mb-px flex gap-1 overflow-x-auto">
            @foreach ($tabs as $key => $tab)
                ...
                <a href="{{ route($tab['route'], $supplier) }}" wire:navigate
                   class="inline-flex shrink-0 items-center gap-2 border-b-2 px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors
                          {{ $isActive ? 'border-[#0d2a4a] text-[#0d2a4a]' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' }}">
                    @if ($key === 'details')
                    ...
                    @if ($tab['badge'] !== null)
                        <span class="inline-flex items-center justify-center rounded-full px-1.5 py-0.5 text-xs font-semibold leading-none
                                     {{ $isActive ? 'bg-[#0d2a4a] text-white' : 'bg-gray-100 text-gray-600' }}">
                            {{ $tab['badge'] }}
                        </span>
                    @endif
                </a>
