        ...
        <div class="sheet">
            ...
            <table class="head">
                <tr>
                    ...
                    <td style="width:36%; text-align:right;">
                        ...
                        <div class="doc-title">{{ $report['title'] ?? 'Classified Client Statement' }}</div>
                        <div class="doc-meta">
                    ...
                    <tr>
                        @foreach ($report['columns'] as $column)
                            @if ($column['key'] !== 'rent_property_count')
                                <th
                                    style="width:
                @if ($column['key'] === 'client_name') 28%
                @elseif($column['key'] === 'sale_property_count') 16%
                @elseif($column['key'] === 'overdue_count') 10%
                @elseif($column['key'] === 'total_paid') 16%
                @elseif($column['key'] === 'total_due') 16%
                @else auto @endif
                        ...
                        <tr @if ($loop->even) class="zebra" @endif>
                            @foreach ($report['columns'] as $column)
                                ...
                                @elseif($column['key'] === 'overdue_count')
                                    <td style="text-align:center;">
                                        <span class="overdue-chip">{{ $row['overdue_count'] ?? 0 }}</span>
                                    </td>
            ...
            <table class="totals">
                <tr>
                    ...
                    <td style="width:14%;">
                        <div class="t-k">Total Overdue</div>
                        <div class="t-v">{{ collect($report['rows'])->sum('overdue_count') }}</div>
                    </td>
                    <td style="width:18%;">
            ...
            <table class="foot">
                <tr>
                    <td style="text-align:left;">{{ config('company.name') }} ·
                        {{ $report['title'] ?? 'Classified Client Statement' }}</td>
